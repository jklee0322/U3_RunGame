...
public class StageCreate : MonoBehaviour
{
    ...
    // 미리 생성할 스테이지 팁의 개수를 설정 (캐릭터가 이동할 때 스테이지가 끊김 없이 이어지도록 함).
    public int preInstantiate;

    // 생성된 스테이지 오브젝트들을 관리하기 위한 리스트.
    public List<GameObject> generatedStageList = new List<GameObject>();

    // Start는 게임 시작 시 최초로 한 번 호출됨. 초기 세팅을 담당.
    void Start()
    {
    ...
    // Update는 매 프레임마다 호출되며, 캐릭터의 위치에 따라 스테이지 업데이트를 관리.
    void Update()
    {
        // 캐릭터의 현재 z 위치를 기준으로, 스테이지의 인덱스를 계산. 스테이지는 stageTip_Size 단위로 구분됨.
        int characPositionIndex = (int)(character.position.z / stageTip_Size);

        // 캐릭터가 현재 생성된 스테이지 범위를 벗어나면 추가로 스테이지를 생성.
        if (characPositionIndex + preInstantiate > currentTipIndex)
        {
            // 캐릭터의 인덱스와 미리 생성할 개수(preInstantiate)를 더해서 새로운 스테이지를 생성.
            UpdateStage(characPositionIndex + preInstantiate);
        }
    }
